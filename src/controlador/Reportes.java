    ...
    public void ReportesVentas() {
        ...
        try {
            ...
            try {
                Connection cn = Conexion.conectar();
                PreparedStatement pst = cn.prepareStatement(
                        "SELECT cv.idCabeceraVenta as id, COALESCE(concat(c.nombre, ' ', c.apellido), 'Compra Directa') as cliente, "
                        + "cv.valorPagar as total, cv.fechaVenta as fecha, cv.estado "
                        + "FROM tb_cabecera_venta as cv "
                        + "LEFT JOIN tb_cliente as c ON cv.idCliente = c.idCliente;");
                ResultSet rs = pst.executeQuery();
                if (rs.next()) {
                    do {
                        tabla.addCell(rs.getString(1));
